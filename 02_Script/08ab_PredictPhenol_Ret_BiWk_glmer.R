...
Phen <- read.csv(file.path(here::here("03_GeneratedData"),"05_ZPPhenologyMetrics.csv"),
  ...
  mutate(DOY_moment = as.POSIXct(DOY_moment, format = "%Y-%m-%d"),
         ...
         # this sets the window for avg predictors
         DOYwinStart = DOY - 30) %>% 
  select(-DOY_moment)
...
Pred_BW <- read.csv(file.path(here::here("03_GeneratedData"),"07b_Predictors_LogedAnoms_BasinAvg_BiWk.csv"),
          ...
          select(-c(MeanIce, Chla, PPLthen10, MesoZPfood))
...
names(PhenPred_SG2) <- c("Taxa", "Year", "Sample_site", "Moment","M", "DOY", "DOYwinStart", 
                         "AvgTemp", "Byth", "EdiblePP","InEdiblePP","Lepto", "plank_per_ha")
...
pdf("05_Figures/08ab_PredictorsCorr4PhenMetrics_Ret_BW_BiWk.pdf")
...
pdf("05_Figures/08ab_PredictorsCorr4PhenMetrics_Ret_SiteGroup_BiWk.pdf")
...
PhenPred_15per <- PhenPred_SG2 %>% 
                  filter(Moment == "per15") %>%
                  # loosing a lot b/c of fish, but no effect
                  select(-plank_per_ha) %>%
...
# note message about warnings
# https://stackoverflow.com/questions/56674534/glmer-warning-parameters-or-bounds-appear-to-have-different-scalings
...
write.csv(as.data.frame(lmm.15per.global.Dredge), 
          file.path(here::here("07_Tables"),"08ab_ModelSelTable_RetPhen_15Per_BiWK_glmer.csv"))

### most likely ----
lmm.15per.MostLik <- glmer(DOY ~ Byth + EdiblePP*Lepto + InEdiblePP + (1|Sample_site),
                          family = Gamma(link = "identity"),
...
plot(ggpredict(lmm.15per.MostLik, "Byth"), residuals = TRUE)
plot(ggpredict(lmm.15per.MostLik, "InEdiblePP"), residuals = TRUE)
plot(ggpredict(lmm.15per.MostLik, c("EdiblePP", "Lepto [-0.68,0.82]")), residuals = TRUE)
plot_model(lmm.15per.MostLik, type = "re")

# https://sscc.wisc.edu/sscc/pubs/MM/MM_DiagInfer.html
# https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html

# note points are data here, not residuals which doesn't work with facets
plot(ggpredict(lmm.15per.MostLik, terms = c("Byth", "Sample_site")), add.data = TRUE, facets = TRUE)
plot(ggpredict(lmm.15per.MostLik, terms = c("EdiblePP", "Lepto", "Sample_site")), add.data = TRUE, facets = TRUE)

saveRDS(lmm.15per.MostLik, here::here("06_Rdat", "08ab_GAMPhenRet_15per_BiWk_glmer.RDS"))
write.csv(PhenPred_15per, here::here("03_GeneratedData", "08ab_GAMPhenRet_15per_BiWk_glmer.csv"))
...
PhenPred_50per <- PhenPred_SG2 %>% 
  filter(Moment == "per50")%>%
  # loosing a lot b/c of fish  < 2, but NS
  select(-plank_per_ha) %>%
  filter(complete.cases(.)) %>% 
  mutate(Sample_site = as.factor(Sample_site))
...
write.csv(as.data.frame(lmm.50per.global.Dredge), 
          file.path(here::here("07_Tables"),"08ab_ModelSelTable_RetPhen_50Per_BiWK_glmer.csv"))
...
saveRDS(lmm.50per.MostLik, here::here("06_Rdat", "08ab_GAMPhenRet_50per_BiWk_glmer.RDS"))
write.csv(PhenPred_50per, here::here("03_GeneratedData", "08ab_GAMPhenRet_50per_BiWk_glmer.csv"))

## 85%  ----

PhenPred_85per <- PhenPred_SG2 %>% 
  filter(Moment == "per85") %>% 
  # loosing a lot b/c of fish, but fish NS
  select(-plank_per_ha) %>%
  filter(complete.cases(.)) %>% 
  mutate(Sample_site = as.factor(Sample_site))
...
write.csv(as.data.frame(lmm.85per.global.Dredge), 
          file.path(here::here("07_Tables"),"08ab_ModelSelTable_RetPhen_85Per_BiWK_glmer.csv"))

### most likely ----
lmm.85per.MostLik <- glmer(DOY ~ Byth + InEdiblePP * Lepto + (1|Sample_site),
                           family = Gamma(link = "identity"),
                           glmerControl(optimizer ='optimx', optCtrl=list(method='nlminb')),
                           data = PhenPred_85per)
...
plot(ggpredict(lmm.85per.MostLik, c("InEdiblePP", "Lepto [-0.71,0.84]")), residuals = TRUE)
plot_model(lmm.85per.MostLik, type = "re")
# https://sscc.wisc.edu/sscc/pubs/MM/MM_DiagInfer.html
# https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html


saveRDS(lmm.85per.MostLik, here::here("06_Rdat", "08ab_GAMPhenRet_85per_BiWk_glmer.RDS"))
write.csv(PhenPred_85per, here::here("03_GeneratedData", "08ab_GAMPhenRet_85per_BiWk_glmer.csv"))

## Peak Biomass  ----

PhenPred_PB <- as.data.frame(PhenPred_BW2) %>% 
  filter(Moment == "PB1") %>% 
  # loosing a lot b/c of fish and not sig
  # select(-plank_per_ha) %>%
  filter(complete.cases(.)) 

### model sel ----
# added plank interaction after data exploration
...
write.csv(as.data.frame(lmm.PB.global.Dredge), 
          file.path(here::here("07_Tables"),"08ab_ModelSelTable_RetPhen_PB_BiWK_glmer.csv"))

### most likely ----
lmm.PB.MostLik <- glm(DOY ~ Byth + InEdiblePP * plank_per_ha,
                           family = Gamma(link = "identity"),
                           data = PhenPred_PB)
...
plot(ggpredict(lmm.PB.MostLik, c("InEdiblePP", "plank_per_ha [-0.74,0.62]")), residuals = TRUE)
# https://sscc.wisc.edu/sscc/pubs/MM/MM_DiagInfer.html
# https://cran.r-project.org/web/packages/DHARMa/vignettes/DHARMa.html


saveRDS(lmm.PB.MostLik, here::here("06_Rdat", "08ab_GAMPhenRet_PB_BiWk_glmer.RDS"))
write.csv(PhenPred_PB, here::here("03_GeneratedData", "08ab_GAMPhenRet_PB_BiWk_glmer.csv"))

# save/load
# save.image(file.path(here::here("04_SavedRImages"),"08ab_PredictPhenol_Ret_BiWk_glmer.RData"))
# load(file.path(here::here("04_SavedRImages"),"08ab_PredictPhenol_Ret_BiWk_glmer.RData"))
